...
// Return TRUE if year is NOT a leap year (refer to AN2543)
uint8_t TIME_notLeapYear(void) {
  if(!(t.year % 100)) return(t.year % 400);
  return(t.year % 4);
}

// Return day of the week (Sakamoto's method, refer to wikipedia)
...
void TIME_send(void) {
  ...
  static char *TIME_days[] = { "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" };
...
int main(void) {
  ...
  while(1) {                                    // loop until forever                         
    ...
    if(ADC0.RESL < 65) TIME_send();             // send time if VCC > 4.3V (65=256*1.1V/4.3V)
  }
}
